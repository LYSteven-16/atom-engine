...
- `height?: number` - 视频高度
- `position?: { x: number; y: number; z?: number }` - 位置
- `radius?: number` - 圆角像素值（默认 0）
- `controls?: boolean` - 是否显示控制条（默认 true）
...
interface WorkplaceConfig {
  ...
  borderWidth?: number;          // 边框宽度 (px)，默认 0
  borderColor?: string;          // 边框颜色，默认 '#000000'

  // 阴影配置
  showShadow?: boolean;          // 是否显示阴影，默认 true
  shadowBlur?: number;           // 阴影模糊半径 (px)，默认 20
  shadowSpread?: number;         // 阴影扩展半径 (px)，默认 0
  shadowColor?: string;          // 阴影颜色，默认 'rgba(0,0,0,0.1)'
  shadowOffsetX?: number;        // 阴影 X 轴偏移 (px)，默认 0
  shadowOffsetY?: number;        // 阴影 Y 轴偏移 (px)，默认 2

  // 渐变与背景图
  gradientType?: 'none' | 'linear' | 'radial';  // 渐变类型
  gradientAngle?: number;         // 线性渐变角度 (deg)，默认 180
  gradientColors?: string[];      // 渐变颜色数组
  gradientStops?: number[];       // 渐变停止点 (0-1)
  backgroundImage?: string;       // 背景图片 URL
  backgroundSize?: string;        // 背景尺寸 (如 'cover', 'contain')
  backgroundPosition?: string;    // 背景位置
  backgroundRepeat?: 'no-repeat' | 'repeat' | 'repeat-x' | 'repeat-y';

  // 溢出与裁剪
  overflow?: 'visible' | 'hidden' | 'scroll' | 'auto';
  clipContent?: boolean;          // 是否裁剪内容，默认 true (即 hidden)
}
