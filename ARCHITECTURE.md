...
private workplace: HTMLElement;               // 工作区 DOM 元素
private workplaceConfig: WorkplaceConfig;      // 工作区当前配置
```
...
#### updateWorkplace(config: WorkplaceConfig)
动态更新工作区样式。该方法采用增量更新策略，将新配置与现有配置合并，并调用内部的 `applyWorkplaceStyles` 重新渲染 DOM。

#### applyWorkplaceStyles(config: WorkplaceConfig)
内部私有方法，将 `WorkplaceConfig` 转换为对应的 CSS 属性。**关键实现**：使用局部箭头函数 `px` 替代独立方法，完全内联像素值转换逻辑：

```typescript
private applyWorkplaceStyles(config: WorkplaceConfig): void {
  const style = this.workplace.style;
  const px = (v, def = '0px'): string => {
...
**设计决策**：将 `px` 辅助函数设为局部变量而非类方法，原因：
1. **消除方法调用开销**：内联代码在打包后不会产生任何 `this` 上下文问题
2. **提高可靠性**：避免 tsup 打包可能导致的任何作用域问题
3. **代码更紧凑**：打包后的产物体积更小

**DOM 挂载顺序**：
`createWorkplace` 方法遵循「先挂载、后样式」的顺序：
1. 首先调用 `appendChild` 将 Workplace 元素添加到父容器
2. 然后调用 `applyWorkplaceStyles` 应用样式

这个顺序对于相对单位（如 `100%`）的正确解析至关重要。如果顺序颠倒，相对尺寸无法正确解析到父容器的尺寸，导致 Workplace 显示异常。
